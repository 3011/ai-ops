...
class AgentResultValidator:
    def validate(
        ...
        *,
        ...
        if diagnosis.hypotheses and not any(item.contradicting_fact_refs or item.support_level == "contradicted" for item in diagnosis.hypotheses):
            issue("NO_CONTRADICTION_CHECK", "$.hypotheses", "没有记录重要假设的反证检查。", "warning")
